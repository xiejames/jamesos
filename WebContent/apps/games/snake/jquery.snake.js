...
var Snake = {
	...
	setup : function() {
		...
		// build map
		Snake.$map = $("#map1");
		Snake.$map.width = 600; // Snake.$map.innerWidth();
		Snake.$map.height = 400; // Snake.$map.innerHeight();
	...
	newGame : function(reset) {
		...
		$('#snake_bckg').css('background', 'url(apps/games/snake/images/wall' + Snake.level + '.gif)');
	...
	animate : function() {			
		...
		Snake.seg[0].addClass('snakehead');				
	},
	...
	advanceLevel : function() {
		...
		//$('#snake_bckg').css('background', 'applications/snake/images/wall' + rand_no + '.gif');
	...
	toggleGrid : function(){
		var background;
		if (!Snake.grid) {
			background = "transparent url(applications/snake/grid_bg.png)";
			Snake.grid = 1;
		} else {
...
Level = [
	...
	[
		{cherries : 10, length : 35, speed : 75},
