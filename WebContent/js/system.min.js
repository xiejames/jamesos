...
function StartHTML5(pageName,title) {
	if(title===undefined){
		title="index";
	}
	if (ChkEl($('#html5test'+pageName))) {
		LoadingStart('Loading '+title+' page ...<br />Please wait.', 'Loading ...');
		ensure({
		...
		function () {
			if(pageName===undefined){
				_html5WinStart('html5',title)
			}else{
				_html5WinStart(pageName,title)
			}
		})
	}
}

function StartWorldTime() {
	if (ChkEl($('#worldtimetest'))) {
		LoadingStart('Loading world time ...<br />Please wait', 'Loading ...');
		ensure({
			js: ["apps/worldtime/startup.js"],
			css: ["apps/worldtime/style.css"]
		},
		function () {
			_worldTimeWinStart()
		})
...
function StartChat(isVideo) {
	if (ChkEl($('#chattest'))) {
		LoadingStart('Loading chat room ...<br />Please wait', 'Loading ...');
		ensure({
			js: ["apps/chat/startup.js"],
			css: ["apps/chat/style.css"]
		},
		function () {
			_chatWinStart(isVideo)
		})
	}
}
function StartRSS() {
	if (ChkEl($('#rsstest'))) {
		LoadingStart('Loading rss ...<br />Please wait', 'Loading ...');
		ensure({
			js: ["apps/rss/startup.js"],
			css: ["apps/rss/style.css"]
		},
		function () {
			_rssWinStart()
		})
...
function StartBlog() {
	if (ChkEl($('#jamesblog'))) {
		LoadingStart('Loading blog window ...<br />Please wait.', 'Loading ...');
		ensure({
			js: "apps/blog/startup.js"
		},
		function () {
			_guishBlogStart()
		})
...
function StartBaiduMap() {
	if (ChkEl($('#baiduMap'))) {
		LoadingStart('Loading map window ...<br />Please wait.', 'Loading ...');
		ensure({
			js: ["apps/baidu/map.js","apps/baidu/startup.js"]
		},
		function () {
			_guishBaiduMapStart()
		})
